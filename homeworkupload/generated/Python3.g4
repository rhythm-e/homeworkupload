...
funcdef: 'def' NAME parameters ':' suite;   //python 的函数定义
...
expr_stmt: testlist ( (augassign testlist) | ('=' testlist)*);//连等 加等/减等/...  or  = a = b = c = 1
augassign: ('+=' | '-=' | '*=' | '/=' | '//=' | '%=' );
...
suite: simple_stmt | NEWLINE INDENT stmt+ DEDENT; //缩进，退出缩进  因为if中可能还有if
test: or_test ;
...
trailer: '(' (arglist)? ')' ;
atom: (NAME | NUMBER | STRING+| 'None' | 'True' | 'False' | ('(' test ')'));   //如果有括号，会把整个元素当作atom  为了提高括号的优先级
testlist: test (',' test)* (',')?;//算式  eg： a,b   a   a+b     //↑此处的test不允许出现逗号，因为此处的括号已经是最高优先级
